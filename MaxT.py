...
def appendData(data1,data2):
    for i in range(len(data1)):
            print "length of bow:",len(data1[i])
            list(data1[i]).append(data2[i])
    return data1
...
# clf  = RandomForestClassifier(n_estimators=5000) #随机森林
...
# joblib.dump(clf,'Final_RF_5000_Step2')   #保存RF训练得到的模型

clf = joblib.load('Final_RF_1000_Step2')


list_cover = clf.predict(test_data_bow_fea)

test_data['PREDICT'] = list_cover
